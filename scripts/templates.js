 ...
 * @param {*} data takes a timeTable object as input 
 */
function _getSearchResultTemplate(data)
{
...
function getSearchResultTemplate(data)
{
    ...
    data.times.forEach(function(time){
        output += '<div class="col-12 m-4 border" id="searhc-item-'+data.id+'-'+time.time.departure+'">\
                    ...
                    <div id="ticket-id-'+ data.id +'-'+ time.time.departure +'" class="form-group row">\
                        <div class="col-md-2">\
                            <label for="ticket-type">Biljettyp:</label>\
                    ...
                    <div class="text-center">\
                    <button class="btn" id="new-ticket-'+ data.id +'-'+ time.time.departure +'" type="button">'+ time.price +'€</button>\
                    </div>\
                </div>';
    });
    return output;
